...
static void runThis() {
    ...
    data.serverPort = 4893;
    char url2[] = {"opc.udp://224.0.0.22:4844/"};// to door_modules
    ...
    pub.addVariableToPublish(VAR_TYPE_BOOLEAN, wg[0], ID_BOOLEAN);
    ...
    pub.addVariableToPublish(VAR_TYPE_BYTE, wg[2], ID_BYTE);


    //Create new channel
    pub.setChannelPort(4843);//to UID_key
    ...
    pub.addVariableToPublish(&UA_TYPES[UA_TYPES_UINT32], wg[1], UA_NODEID_NUMERIC(1, 5555));
    ...
    for(size_t j = 0; j < networkMessage.payloadHeader.dataSetPayloadHeader.count; j++) {
        ...
        for(int i = 0; i < dsm->data.keyFrameData.fieldCount; i++) {
            ...
            if(currentType == &UA_TYPES[UA_TYPES_BYTE]) {
                ...
                UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"Send HF Data key");
                ...
                UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"Send calculated Signature");
                ...
                UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"CAN Entry Validation");
                ...
                static UA_UInt32 uInt32Val = 0;
                uInt32Val += 1000;
                UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"HW Trigger / WAKE-UP");

                // Change Value for Publish
                //pub.writeValueToVariable(VAR_TYPE_BOOLEAN, &doubleValue, ID_BOOLEAN);
                pub.writeValueToVariable(&UA_TYPES[UA_TYPES_UINT32], &uInt32Val, UA_NODEID_NUMERIC(1, 5555));

                UA_Server_setWriterGroupOperational(pub.server, wg[2]);// Do Publish
                UA_Server_setWriterGroupOperational(pub.server, wg[1]);// Do Publish
                UA_Server_setWriterGroupDisabled(pub.server, wg[2]);//Stop Publish
...
int main(int argc, char **argv) {
    ...
    char url[]="opc.udp://224.0.0.22:4842/";
